...
for num in num_clusters:
    spectral_clustering = SpectralClustering(n_clusters=num, assign_labels="discretize", random_state=0, n_init=10).fit(data)
    cluster_labels = []

    with open(f"results_{num}_sp_clusters.txt", "w") as f:
        for i in range(num):
            cluster_data = data[spectral_clustering.labels_ == i]
            cluster_labels.append(labels[spectral_clustering.labels_ == i])
